...
CREATE INDEX IF NOT EXISTS idx_expenses_date_range ON expenses(date) WHERE date >= CURRENT_DATE - INTERVAL '1 year';

-- 7. 创建函数：自动更新updated_at字段
...
-- 注意：这些数据将只对执行脚本的用户可见
/*
INSERT INTO budgets (user_id, amount, year, month) VALUES 
  (auth.uid(), 3000.00, EXTRACT(YEAR FROM CURRENT_DATE)::INTEGER, EXTRACT(MONTH FROM CURRENT_DATE)::INTEGER)
  ON CONFLICT (user_id, year, month) DO NOTHING;

INSERT INTO expenses (user_id, amount, category, description, payment_method) VALUES 
  (auth.uid(), 25.50, 'food', '午餐 - 快餐', 'card'),
  (auth.uid(), 15.00, 'transport', '地铁费用', 'cash'),
  (auth.uid(), 120.00, 'shopping', '日用品采购', 'online')
...
WHERE table_schema = 'public' 
  AND table_name IN ('budgets', 'expenses')
...
WHERE schemaname = 'public' 
  AND tablename IN ('budgets', 'expenses');
...
-- 执行完毕提示
SELECT '✅ 数据库初始化完成！所有表、索引、RLS策略和触发器已创建。' AS message; 