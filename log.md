...
#### 问题报告
前端反馈：没有部署相关OCR API端点

#### 问题分析
通过检查发现：
1. **API文档已更新**：`docs/API.md`中包含了新的OCR端点
2. **路由配置缺失**：`src/routes/ocr.js`中缺少`/parse-auto`路由
3. **控制器方法缺失**：`src/controllers/ocrController.js`中缺少`parseTextAndAutoCreate`方法
4. **路由列表未更新**：`src/app.js`中的调试路由列表未包含新端点

#### 修复内容

##### 1. 添加OCR自动创建控制器方法
在`src/controllers/ocrController.js`中添加：
```javascript
/**
...
#### 验证结果
- ✅ 服务器重启后新端点可用
- ✅ `/api/debug/routes`显示新的OCR端点
- ✅ API端点正确要求认证
- ✅ 路由配置完整

#### 新增API功能
1. **`POST /api/ocr/parse-auto`**：
   - 智能解析OCR文本
   - 置信度≥0.85时自动创建支出记录
   - 置信度较低时返回解析结果供确认
   - 支持自定义置信度阈值

2. **`GET /api/ocr/shortcuts/generate`**：
   - 生成iOS快捷指令JSON配置
   - 包含完整的拍照→OCR→API调用流程
   - 提供详细的设置指导
